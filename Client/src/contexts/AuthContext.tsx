// src/contexts/AuthContext.tsx - Vérification du token
...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      // ⚠️ Pour l'instant, on fait confiance au token stocké
      // En production, vous pourriez vouloir valider le token côté serveur
      setState({
        user: {
          id: 1,
          username: 'admin',
  ...
  const switchCountry = (country: CountryCode) => {
    ...
    if (state.user) {
      ...
      setState(prev => ({
        ...prev,
        user: prev.user ? { ...prev.user, country } : null
      }));
    }
  };
