...
METRICS = {
    "cases": (
        "total_confirmed",
        "New cases",
    ),  # Utiliser les colonnes "New" pour les totaux réels
    "deaths": (
        "total_deaths",
        "New deaths",
    ),  # Ces colonnes contiennent les nouveaux cas par jour
...
def trend(
    metric: str, days: int = Query(30, ge=1, le=365), db: Session = Depends(get_db)
):
...
def validate_data(country: str = "usa", db: Session = Depends(get_db)):
    ...
    if result:
        return {
            ...
            "data_looks_valid": result["max_cumulative_deaths"]
            < 2000000,  # Seuil raisonnable
        }
